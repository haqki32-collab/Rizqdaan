...
const ChatPage: React.FC<ChatPageProps> = ({ currentUser, targetUser, onNavigate }) => {
  ...
  // New State for Sidebar Tabs & Notifications
  ...
  // 1. Initialize or Load Conversation if targetUser is provided
  useEffect(() => {
    const initChat = async () => {
      if (targetUser && db) {
        // Generate a consistent ID based on sorted user IDs to ensure 1-on-1 uniqueness
        const sortedIds = [currentUser.id, targetUser.id].sort();
        const convId = `${sortedIds[0]}_${sortedIds[1]}`;
        
        setActiveConversationId(convId);
        setActiveTab('messages'); // Force switch to messages if navigating to a specific chat
      }
    };
    initChat();
  }, [targetUser, currentUser.id]);

  // 2. Listen to Conversations List
  useEffect(() => {
  ...
  // 3. Listen to Notifications (For Alerts Tab)
  useEffect(() => {
      if (!currentUser || !db) return;
  ...
  // 4. Listen to Messages & Mark as Read
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      if (unreadIds.length > 0) {
        ...
        unreadIds.forEach(id => {
            ...
            updateDoc(msgRef, { read: true }).catch((e:any) => console.error("Error marking read:", e.message));
        });

        // Reset unread count for current user in Conversation doc
        const convRef = doc(db, 'conversations', activeConversationId);
        updateDoc(convRef, {
            [`unreadCounts.${currentUser.id}`]: 0
        }).catch((e:any) => console.error("Error updating unread count:", e.message));
      }
    });
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
        ...
        // Use targetUser name if available (for new chats), otherwise find in existing convos
  ...
  // Helper to get other user's name from conversation
  ...
  return (
    <div className="flex flex-col md:flex-row h-[calc(100dvh-82px)] md:h-[calc(100vh-140px)] bg-white dark:bg-dark-surface rounded-xl shadow-lg overflow-hidden animate-fade-in border border-gray-100 dark:border-gray-700">
      ...
      {/* Main Chat Area (Only Visible when Chat ID is active OR on Desktop when Tab is Alerts) */}
      <div className={`w-full md:w-2/3 flex flex-col h-full ${!activeConversationId ? 'hidden md:flex' : 'flex'}`}>
        
        {/* If viewing alerts on desktop, we can show a placeholder or keep previous chat context. 
            For simplicity, if tab is 'alerts', we show a placeholder unless a chat was already open.
        */}
        {activeTab === 'alerts' && !activeConversationId ? (
             <div className="flex flex-col items-center justify-center h-full text-gray-400 p-6 text-center bg-gray-50/50 dark:bg-gray-900/50">
                ...
                <p className="text-sm mt-2 max-w-sm">
                    Admin announcements, wallet updates, and listing approvals appear in the alerts tab. Click on an alert to mark it as read.
                </p>
            </div>
            ...
            <>
                        ...
                        return (
                            <div key={msg.id} className={`flex w-full ${isMe ? 'justify-end' : 'justify-start'}`}>
                                <div className={`relative max-w-[85%] sm:max-w-[70%] px-3 pt-2 pb-6 rounded-xl shadow-sm min-w-[80px] ${
                                    ...
                                    {/* Timestamp & Read Receipts at bottom right inside bubble */}
                                             ...
                                             <span className={`${msg.read ? 'text-blue-300' : 'text-gray-400'}`}>
                                                {msg.read ? (
                                                    ...
                                                    <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                                                        <path d="M0.41 13.41L6 19l1.41-1.41L1.83 12 0.41 13.41zm22.24-9.06l-12 12L9.07 14.83 7.66 16.24 10.66 19.24 24.07 5.76 22.65 4.35zM18 7l-1.41-1.41-6.34 6.34 1.41 1.41L18 7z" />
                                                    </svg>
                                                ) : (
                                                    // Single Tick (Sent)
                ...
                <form onSubmit={handleSendMessage} className="p-3 pr-4 bg-white dark:bg-dark-surface border-t border-gray-200 dark:border-gray-700 flex gap-2 flex-shrink-0 items-center">
                    ...
                    <button type="submit" disabled={!newMessage.trim()} className="bg-primary text-white w-12 h-12 rounded-full hover:bg-primary-dark disabled:opacity-50 transition-all shadow-md flex items-center justify-center flex-shrink-0 active:scale-95">
                        {/* Centered Right Facing Send Arrow - Adjusted for visual balance */}
