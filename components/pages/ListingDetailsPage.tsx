...
import { WhatsAppDealCardModal } from '../common/WhatsAppDealCardModal';
    ...
    <div className="bg-gray-50 dark:bg-black min-h-screen pb-10 animate-fade-in overflow-x-hidden">
      ...
      <SectionWrapper className="!bg-primary/5 dark:!bg-primary/10">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
              <button 
                id="listing-chat-now-btn"
                onClick={() => { 
                    trackConversation('chat');
                    const targetId = listing.vendorId || (listing.vendor && (listing.vendor as any).id) || (vendorData?.id) || 'vendor_' + (listing.vendorName ? listing.vendorName.replace(/[^a-zA-Z0-9]/g, '_').toLowerCase() : 'seller');
                    const targetName = vendorData?.shopName || listing.vendorName || 'Seller';
                    onNavigate('chats', { 
                        targetUser: { id: targetId, name: targetName },
                        listing: listing
                    }); 
                }} 
                className="flex items-center justify-center gap-3 h-14 bg-white dark:bg-dark-surface text-primary dark:text-white font-black rounded-2xl active:scale-95 transition-all border-2 border-primary shadow-sm hover:bg-primary/5 cursor-pointer"
              >
              ...
              <a 
                ...
                href={`https://wa.me/${(vendorData?.phone || listing.contact.whatsapp).replace(/[^0-9]/g, '')}?text=${encodeURIComponent(`Assalam o Alaikum! I am inquiring about "${listing.title}" (Rs. ${listing.price.toLocaleString()}) on RizqDaan.\nProduct Link: ${window.location.href}`)}`} 
                target="_blank" 
                rel="noreferrer"
          ...
          {/* 📱 Quick One-Click WhatsApp Share Bar & Image Poster Maker */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-3.5 bg-amber-50 hover:bg-amber-100 dark:bg-amber-950/40 text-amber-800 dark:text-amber-300 font-black text-xs uppercase tracking-wider rounded-2xl border border-amber-300/50 transition-all active:scale-[0.99]"
            >
              <span>🔥</span>
              <span>{isUrdu ? 'وائرل ڈیل کارڈ و پوسٹر بنائیں' : 'Make Viral Deal Poster'}</span>
            </button>
          </div>
      </SectionWrapper>
      ...
      {/* 📱 WHATSAPP 1-CLICK DEAL CARD & POSTER MODAL */}
