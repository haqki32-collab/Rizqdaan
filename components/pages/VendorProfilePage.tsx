...
interface VendorProfilePageProps {
  ...
  listings: Listing[];
  // Relaxed type to prevent "Failed to load" type mismatch errors
...
const VendorProfilePage: React.FC<VendorProfilePageProps> = ({ vendorId, currentUser, listings, onNavigate }) => {
  ...
  const [badgeThreshold, setBadgeThreshold] = useState(5); // Default, updates from DB

  // Filter listings for this vendor
  ...
  // Real-time Vendor Data Fetching
  useEffect(() => {
    if (!vendorId || !db) {
        setLoading(false);
        return;
    ...
    const unsubscribe = onSnapshot(docRef, (docSnap) => {
        if (docSnap.exists()) {
            ...
            // SAFETY: Provide defaults to prevent crashes if fields are missing
            setVendor({ 
                id: docSnap.id, 
  ...
  // Derived state to check if current user is following
  ...
  return (
    <div className="animate-fade-in pb-20">
      ...
      <div className="bg-white dark:bg-dark-surface pb-6 shadow-sm mb-6">
        ...
        <div className="container mx-auto px-4 relative">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-end -mt-16 md:-mt-20 mb-4 relative z-20">
                ...
                {/* Action Buttons (Desktop & Mobile Layout) */}
            ...
            {/* Vendor Name & Details (Below Cover) */}
            <div className="mt-2">
                 <h1 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white flex flex-wrap items-center gap-2">
                    {vendor.shopName}
                    {vendor.isVerified && (
                        <svg className="w-6 h-6 text-blue-500" viewBox="0 0 24 24" fill="currentColor" title="Verified Vendor">
                            <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z" />
                        </svg>
                    )}
                    {hasStarBadge && (
                        <div className="flex items-center bg-yellow-100 text-yellow-800 text-xs font-bold px-2 py-0.5 rounded-full border border-yellow-200" title="Community Star">
      ...
      {/* 2. Stats Section (Clean Professional Look) */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white dark:bg-dark-surface p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                        ...
                        <p className="text-gray-800 dark:text-white text-sm font-medium mb-3">{vendor.shopAddress}</p>
                        <a 
                            href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(vendor.shopAddress || '')}`} 
                            target="_blank" 
                            rel="noreferrer"
                            className="w-full block text-center py-2 px-4 bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-200 text-xs font-bold rounded border border-gray-200 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors"
                        >
                            View on Maps
                        </a>
                    </div>
